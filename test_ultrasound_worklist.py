...
def test_worklist_sync():
    ...
    print("=" * 60)
    print("TEST CHUC NANG DONG BO WORKLIST - MÁY SIÊU ÂM")
    print("=" * 60)
    ...
    # 1. Kiem tra cau hinh may sieu am
    print("\n1. KIEM TRA CAU HINH MÁY SIÊU ÂM:")
    print("-" * 60)
    sync_service = get_voluson_sync_service()
    ultrasound_ip = sync_service.Maysieuam_ip
    ultrasound_port = sync_service.Maysieuam_port
    ultrasound_ae_title = sync_service.Maysieuam_ae_title
    ...
    if appointments:
        ...
        for apt in appointments:
            ...
            cursor.execute("""
                SELECT Maysieuam_synced, Maysieuam_sync_time
                FROM appointment
                WHERE id = ?
            """, (apt[0],))
            sync_status = cursor.fetchone()
            if sync_status:
                print(f"    Synced: {sync_status[0]}, Time: {sync_status[1]}")
    else:
        print("Khong tim thay appointment nao co dich vu sieu am!")
    
    # 4. Test ket noi may sieu am
    print("\n4. TEST KET NOI MÁY SIÊU ÂM:")
    print("-" * 60)
    if sync_service.test_connection():
        ...
        print("   - IP và Port của máy siêu âm đúng")
